...
public class CadastrarUsuarioBean implements Serializable {
	...
	public void salvar() {
		log.info("Tentando salvar utilizador: cpf=" + usuario.getCpf() + ", nome=" + usuario.getNome());

		Usuario logado = loginBean.getUsuarioLogado();

		if (Perfil.PACIENTE.equals(logado.getPerfil()) && !Perfil.PACIENTE.equals(usuario.getPerfil())) {
		...
		addMensagem(FacesMessage.SEVERITY_INFO, "Utilizador gravado com sucesso!", novoUsuario.getNome());
		limpar();
	}
	...
	public void excluir() {
		...
		try {
			log.info("Excluindo utilizador: nome=" + usuario.getNome() + " por admin cpf=" + logado.getCpf());

			usuarioService.excluir(usuario);
			this.usuarios = usuarioService.buscarTodos();

			addMensagem(FacesMessage.SEVERITY_INFO, "Utilizador " + usuario.getNome() + " excluído com sucesso.", null);

		} catch (Exception e) {
			log.error("Erro ao excluir utilizador id=" + usuario.getId(), e);
			addMensagem(FacesMessage.SEVERITY_ERROR, "Não foi possível excluir o utilizador.", null);
		}
	}
